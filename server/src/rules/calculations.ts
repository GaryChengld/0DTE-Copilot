...
export function computeSpreadCredit(
  spx: number, direction: Direction, vix: number, remainingHours: number, r = 0.04
  ...
  const sigma = vix / 100
  const credit = direction === 'bear_call'
...
export function extractGexData(marketSummary: unknown): GexData | null {
  ...
  if (typeof gex.gamma_flip !== 'number' || typeof gex.call_wall !== 'number' || typeof gex.put_wall !== 'number') return null
